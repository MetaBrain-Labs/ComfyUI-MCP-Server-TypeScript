...
4. **注册工具**：使用选定的名称调用 `cui_mount_dynamic_tool`。  
 _ `workflow_name`: 第一阶段确定的确切名称字符串（作为 ID）。  
 _ `extension_name`: 生成一个唯一的简短后缀（正则表达式：`^[a-z][a-z0-9_]*$`）。示例：`flux_p`。  
5. **关键停止序列 (CRITICAL STOP SEQUENCE)**：在调用 `cui_mount_dynamic_tool` 后立即执行：  
 _ **终止生成**。  
...
1. 调用 `cui_mount_dynamic_tool(workflow_name="flux_portrait_v2", extension_name="flux_p")`。
2. **[停止生成]**
